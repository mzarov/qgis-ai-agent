...
Нужны две библиотеки: `requests` для запросов и `keyring` для хранения ключа.

`requests` входит во все проверенные сборки QGIS. `keyring` есть в QGIS 4 под
macOS; если в вашей сборке его нет или на Linux не запущен сервис секретов
...
```bash
"/путь/из/консоли" -m pip install requests keyring
```
