...
При изменении кода в репозитории перезагрузите плагин: **Плагины → Управление и установка плагинов → Установлено → снять галочку с QGIS AI Agent → поставить снова** (или перезапуск QGIS).

## 5. Зависимости Python (requests, keyring)

Функции «Настройки» и запросы к ИИ требуют библиотек `requests` и `keyring`. **Плагин загружается и без них** (кнопка «Создать тестовый макет» работает), но при первом сохранении ключа или проверке подключения появится сообщение с инструкцией по установке.
