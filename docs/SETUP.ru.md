...
PyQGIS недоступен. С Python 3.12+ и Poetry 2.1.3 выполните `poetry install`: команда
установит зафиксированные инструменты разработки, не устанавливая плагин как
Python-пакет. Затем выполните `poetry run ruff check .`,
`poetry run ruff format --check .` и `poetry run mypy`. Строгая проверка mypy
покрывает конфигурацию, загрузку навыков, базовый контракт инструментов, проверку
аргументов и политику эффектов Processing; остальное приложение пока не входит в эту область.

Для покрытия ветвей выполните
`poetry run coverage run -m unittest discover -s tests -t .` и
`poetry run coverage html`, затем откройте `build/coverage/index.html`. CI
публикует тот же HTML и XML в артефакте `fast-suite-branch-coverage`. Отчёты
относятся к быстрому набору с заглушками QGIS и не включают отдельные процессы
настоящего QGIS. Пропущенные ветви помогают выбирать регрессионные тесты;
обязательного процента покрытия нет.

В CI также запускаются `tests/real_qgis_smoke.py` для установленного ZIP и
