...
публичные ответы DNS, закрепляет проверенный IP при проверке исходного TLS-имени
и следует только редиректам в рамках origin. Сторонний HTTP-клиент не нужен;
несовпадение маршрутов прокси QGIS блокируется, а не обходится.
