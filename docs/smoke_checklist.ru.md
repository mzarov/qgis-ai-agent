...
Ручная проверка в живом QGIS дополняет быстрые модульные тесты и проверки
установки и рабочих сценариев в настоящем QGIS в CI. Этот список нужен для
интерактивного поведения и внешнего вида за пределами автоматических сценариев.
Воспроизводимый сбой API или жизненного цикла по возможности закрепляйте
автоматическим регрессионным тестом.
...
166. **Место превращается в bbox через одобренный сервис.** В настройках выберите
     Photon или укажите разрешённый тестовый Nominatim-сервис, затем попросите
     «Скачай кафе в Дивноморском» → в плане виден хост геокодера; после отдельного
     подтверждения агент передаёт полученный bbox в download_osm.
167. **Вопрос не о проекте уходит в веб.** «Какой EPSG-код у UTM-зоны
     Казани?» → в плане названы DuckDuckGo/Wikipedia; после подтверждения
     search_web отрабатывает, ответ называет ссылку-источник.
168. **Вставленная ссылка читается.** Вставить URL документации и спросить, о
     чём она → в плане виден публичный HTTPS-хост; после подтверждения fetch_url
     возвращает читаемый текст, ответ цитирует страницу, а не галлюцинацию.
...
170. **Публичный геокодер OSMF остаётся заблокирован.** Попытка сохранить
     `https://nominatim.openstreetmap.org` как свой Nominatim отклоняется до
     сетевого запроса со ссылкой на правила использования OSMF.

## Новые возможности и журнал аудита

171. **Аннотации проходят полный цикл.** Попросить текстовую заметку и маркер →
     оба ждут в плане, появляются после **Применить**, видны в
     `list_annotations` и удаляются по id.
172. **3D-вид открывается в заявленных границах.** Попросить именованный 3D-вид
...
173. **Журнал прогона видим и точен.** После применения плана чат сообщает путь
...
174. **Gemini использует общий диалект.** Выбрать пресет Google Gemini, указать
...
176. **Попап по `/` показывает скиллы и вставляет выбранный.** Набрать `/` в
     композере → над полем список всех встроенных скиллов с однострочниками;
     `/o` сужает до совпадений по префиксу; ↓ и Tab вставляют `/osm ` с курсором
     после пробела; Esc закрывает, не трогая текст.
177. **Скилл из слэш-команды загружен до первого хода.** Отправить
     `/osm кафе в Казани` → в ленте нет шага «Загружаю знания: osm», первое
     действие уже `download_osm`, а пузырь пользователя сохраняет исходный текст
     `/osm …`.
178. **Локальный скилл проходит весь путь.** Настройки → Скиллы → *Создать
     пример* → папка открывается по кнопке, пример в списке; изменить
     description → он появляется в `/` с пометкой *локальный*, и агент может
     сделать `load_skill`; добавить неизвестный инструмент → страница показывает
     проблему, а скилл загружается только с известными.
179. **Неизвестная команда отклоняется, а не отправляется.** `/nope сделай` →
     одно системное сообщение со списком доступных команд; до модели ничего не
     доходит.
