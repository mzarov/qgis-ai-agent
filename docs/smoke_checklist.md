...
- `python3 tools/smoke_renderers.py` — разбор одиночного символа, категорий,
  градаций и правил. Ловит `AttributeError` на методах, которых у рендерера нет:
  статические проверки такое не видят.
