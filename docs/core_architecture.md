...
- `qgis_tools/` — тулы по доменам: `inspect/`, `processing/`
...
## Валидация до очереди

Write исполняется **после** конца цикла, поэтому вернуть ошибку модели в этот
момент уже некому. Всё, что можно проверить по аргументам, проверяется в
`BaseTool.validate` при постановке в очередь — там цикл ещё жив, и агент
успевает перестроить план.
...
Рост промпта по мере загрузки: 2034 → 3454 → 6140 символов.
