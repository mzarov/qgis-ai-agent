...
class BaseTool(ABC):
    ...
    def validate(self, params: dict[str, Any]) -> None:
        """
        Проверяет вызов до постановки в очередь. Бросает ValueError с объяснением,
        если аргументы заведомо дадут негодный результат.

        Вызывается внутри агентного цикла, поэтому ошибка возвращается модели, и
        та успевает перестроить план. Проверять здесь стоит только то, что видно
        по аргументам и состоянию проекта, без изменения чего-либо.
        """
        return None
