...
Внутри домена общее живёт в своём файле: у `style/` это `apply.py` — поиск слоя,
разбор цвета, палитры, перерисовка. Пишущие тулы стиля не дублируют эту логику.
...
Механика общая: `style/properties.py` — `StyleProperty` и `PropertySet` с
приведением значений, проверкой диапазонов и подсказками по опечаткам. Каталоги
доменов — `label_catalogue.py` и `symbol_catalogue.py`, оба отдаются одним
`describe_style_options`.
