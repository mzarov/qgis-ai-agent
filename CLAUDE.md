...
```bash
python3 tools/smoke_renderers.py
```
...
Третья прогоняет разбор всех четырёх типов рендерера на подставных объектах.
Она нужна там, где статические проверки бессильны: `AttributeError` от обращения
к методу, которого у объекта нет, виден только при исполнении. `tools/qgis_stub.py`
подменяет `qgis.*`, поэтому проверки работают без установленного QGIS.
