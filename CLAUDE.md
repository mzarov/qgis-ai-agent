...
- Зависимости: Poetry, Python `^3.12`. Плагин крутится внутри QGIS, поэтому каждая
  ...
  крайней необходимости; сейчас их две — `requests` и `keyring`. Сетевые запросы
  изнутри тулов идут через `QgsBlockingNetworkRequest`: он крутит событийный цикл,
  поэтому интерфейс QGIS не замерзает, а правило главного потока не нарушается.
...
`pyproject.toml` наверху, пакет — в `src/`. Корень сборки —
`src/qgis_ai_agent/plugin.py`: единственный модуль, которому можно
импортировать и `core`, и `ui`. Направление слоёв (`ui → core → qgis_tools`,
обратно нельзя) стережёт `tests/test_layering.py`.
