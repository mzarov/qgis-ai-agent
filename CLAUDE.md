...
Скрипт вытаскивает литералы из `tr()`/`tr_n()` разбором AST, дописывает новые в
`qgis_ai_agent/translations/qgis_ai_agent_ru.ts`, сохраняя уже сделанные переводы,
и компилирует `.qm`, если найдёт `lrelease` (или укажи путь через `LRELEASE=...`).
`pylupdate5` из штатного рецепта QGIS вызовы `tr()` находит, но не годится по
трём измеренным причинам: не-ASCII в исходнике он читает как latin-1 (`…` и `■`
приезжают мусором, и перевод потом не совпадает), `tr_n` не видит вовсе — то есть
множественное число теряется, — а контекст ставит `@default` вместо
`QgisAiAgent`, под которым строки ищутся в рантайме.
